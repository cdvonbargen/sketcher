...
{
    auto path = std::filesystem::path("..") / "test" / "testfiles" / filename;
    if (std::getenv("SCHRODINGER_SRC")) {
        path = std::filesystem::path(std::getenv("SCHRODINGER_SRC")) /
               "mmshare" / "test" / "testfiles" / "rdkit_extensions" / filename;
    }
...
void set_default_monomer_db_path()
{
    ...
    if (std::getenv("SCHRODINGER_SRC")) {
        path = std::filesystem::path(std::getenv("SCHRODINGER_SRC")) /
               "mmshare" / "data" / "helm" / "core_monomerlib.db";
    }
    if (!std::filesystem::exists(path)) {
