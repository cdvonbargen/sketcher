...
test.describe('ported tst_move_mode and tst_select_mode_active_selection', () => {
  ...
  test('rectangle selection enables and clears selection actions', async ({ page }) => {
    ...
    expect((await requireRect(page, 'widget:clear_selection_btn')).enabled).toBe(false);
    expect((await requireRect(page, 'widget:invert_selection_btn')).enabled).toBe(false);
    ...
    await expect
      .poll(async () => (await requireRect(page, 'widget:clear_selection_btn')).enabled)
      .toBe(true);
    expect((await requireRect(page, 'widget:invert_selection_btn')).enabled).toBe(true);
    await page.mouse.move(0, 0);
